...
class CircularLinkedList:
    ...
    def __CountNodesAndRemove(self, m):
        
        for i in range(m - 1):
            self.atual = self.atual.next
        
        proximo = self.atual.next
    
        self.Remove(self.atual.item)
        
        self.atual = proximo

    def Josephus(self, m):
        
        self.atual = self.final.next
        
        while self.count > 1:
            self.__CountNodesAndRemove(m)
...
def main():
    ...
    while opcao != 11:
        ...
        if opcao == 1:
            ...
            resultado = x.Josephus(m)
            print(f"A pessoa que teve a vida poupada estava na posição: {resultado}.")
